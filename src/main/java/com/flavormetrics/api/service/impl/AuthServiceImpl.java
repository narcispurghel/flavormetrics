...
public class AuthServiceImpl implements AuthService {
  ...
  @Override
  public UserDetailsImpl authenticate(
    LoginRequest req,
  ...
  public String logout(HttpServletResponse response) {
    response.addHeader(
      ...
      generateLogoutCookie(ACCESS_TOKEN_NAME).toString()
    );
    response.addHeader(
      HttpHeaders.SET_COOKIE,
      generateLogoutCookie(REFRESH_TOKEN_NAME).toString()
    );
    return "Logout success!";
  }

  private ResponseCookie generateLoginCookie(
    String name,
    String value,
    JwtTokens type
  ) {
