...
public class RecipeServiceImpl implements RecipeService {
  ...
  public RecipeDto updateById(UUID id, AddRecipeRequest request) {
    ...
    Set<Allergy> allergies = Optional.of(request)
      ...
      .orElse(Set.of());
    Set<Tag> tags = Optional.of(request)
  ...
  @Override
  @Transactional(readOnly = true)
  public DataWithPagination<List<RecipeDto>> findAll(
  ...
  @Override
  @Transactional(readOnly = true)
  public DataWithPagination<Set<RecipeDto>> getRecommendations(
    final int pageNumber,
    int pageSize
  ) {
    ...
    Pageable pageable = PageRequest.of(pageNumber, pageSize);
    return Optional.of(
      recipeRepository.findAllRecommendations(principal.id(), pageable)
    )
      .map(p -> {
  ...
  public RecipeDto updateRecipeImageById(UUID id, UploadImage request) {
    ...
    return recipeRepository
      ...
      .map(r -> {
        if (
          r.getUser() == null || !r.getUser().getId().equals(principal.id())
        ) {
          throw new UnAuthorizedException(
            "You're not authorized to perform this operation"
          );
