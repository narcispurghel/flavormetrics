...
@WebMvcTest(controllers = AdminController.class,
        excludeFilters = @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE, classes = {JwtAuthenticationFilter.class, CsrfFilter.class}))
...
class AdminControllerTest {
    ...
    void getUserById() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/users/%s".formatted(userDto.getId().toString()))
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpectAll(
    ...
    void lockUserById() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.patch("/api/v1/users/lock/%s".formatted(userDto.getId().toString()))
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpectAll(
                        MockMvcResultMatchers.status().isOk(),
                        MockMvcResultMatchers.jsonPath("$.id").value(userDto.getId().toString()),
                        MockMvcResultMatchers.jsonPath("$.isAccountNonLocked").value(userDetails.isAccountNonLocked()),
                        MockMvcResultMatchers.jsonPath("$.email").value(userDto.getEmail())
                );
    }
    ...
    void unlockUserById() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.patch("/api/v1/users/unlock/%s".formatted(userDto.getId().toString()))
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpectAll(
    ...
    void deleteUserById() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.delete("/api/v1/users/delete/%s".formatted(userDto.getId().toString()))
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON))
                .andDo(print())
